...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Fine-tune SD inpainting on clean/damaged/mask pairs."
    )
    ...
    parser.add_argument("--bootstrap_outputs_dir", default=None)
    parser.add_argument(
        "--bootstrap_probability",
        type=float,
        default=0.0,
        help="Use restored outputs as conditioning images with this probability.",
    )
    parser.add_argument("--output_dir", default="checkpoints/inpaint_lora")
    ...
    parser.add_argument("--train_mode", default="lora", choices=["lora", "full_unet"])
    parser.add_argument("--rank", type=int, default=8)
    parser.add_argument("--lora_alpha", type=int, default=16)
    parser.add_argument("--lora_dropout", type=float, default=0.0)
    parser.add_argument("--target_modules", default="to_q,to_k,to_v,to_out.0")
...
def main() -> None:
    ...
    dataset = PairedInpaintDataset(
        args.train_root,
        resolution=args.resolution,
        default_caption=args.default_caption,
        captions_dir=args.captions_dir,
        bootstrap_outputs_dir=args.bootstrap_outputs_dir,
    ...
    progress = tqdm(
        ...
        desc=f"inpaint-{args.train_mode}",
    )
    for _ in range(args.num_train_epochs):
        unet.train()
        for batch in dataloader:
            with accelerator.accumulate(unet):
                clean = batch["pixel_values"].to(accelerator.device, dtype=weight_dtype)
                conditioning = batch["conditioning_pixel_values"].to(
                    accelerator.device, dtype=weight_dtype
                )
                masks = batch["mask_values"].to(accelerator.device, dtype=weight_dtype)
                masked_conditioning = conditioning * (1.0 - masks)

                latents = vae.encode(clean).latent_dist.sample()
                latents = latents * vae.config.scaling_factor
                masked_latents = vae.encode(masked_conditioning).latent_dist.sample()
            ...
            if accelerator.sync_gradients:
                ...
                if (
                    ...
                    if args.train_mode == "lora":
                        save_lora_weights(
                            checkpoint_dir, accelerator.unwrap_model(unet)
                        )
    ...
    if accelerator.is_main_process:
        ...
        save_training_metadata(
            args.output_dir,
            {
                ...
                "resolution": args.resolution,
                "rank": args.rank if args.train_mode == "lora" else None,
                "lora_alpha": args.lora_alpha if args.train_mode == "lora" else None,
                "target_modules": args.target_modules if args.train_mode == "lora" else None,
        ...
        print(f"Saved {args.train_mode} weights to {args.output_dir}")
